...
const PillNav = ({
  ...
  const resolvedPillTextColor = pillTextColor ?? '#8ca0b8';
  ...
  const handleEnter = (i: number) => {
    ...
    activeTweenRefs.current[i] = tl.tweenTo(tl.duration(), { duration: 0.3, ease, overwrite: 'auto' }) as unknown as gsap.core.Tween;
  };
  ...
  const handleLeave = (i: number) => {
    ...
    if (!tl) return;
    activeTweenRefs.current[i]?.kill();
    activeTweenRefs.current[i] = tl.tweenTo(0, { duration: 0.2, ease, overwrite: 'auto' }) as unknown as gsap.core.Tween;
  };
